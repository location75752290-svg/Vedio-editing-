...
/**
 * SecretManager reads API keys and configuration safely from assets JSON files.
 * It provides fallback mechanisms and runtime checks to ensure safe execution.
 */
object SecretManager {
    ...
    /**
     * Checks if a valid key is provided for the given service.
     */
    fun hasValidKey(context: Context, service: ApiService): Boolean {
        return when (service) {
            ApiService.GEMINI -> !getGeminiApiKey(context).isNullOrBlank()
            ApiService.PIXABAY -> !getPixabayApiKey(context).isNullOrBlank()
            ApiService.PEXELS -> !getPexelsApiKey(context).isNullOrBlank()
            ApiService.FIREBASE -> !getFirebaseConfig(context).isNullOrBlank()
        }
    }
