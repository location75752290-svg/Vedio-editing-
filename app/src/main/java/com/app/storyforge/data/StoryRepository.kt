package com.app.storyforge.data
...
class StoryRepository(private val context: Context) {
    ...
    /**
     * Reads the Gemini API key from assets/secrets_gemini.json
     */
    private fun getApiKey(): String {
        return try {
            val jsonString = context.assets.open("secrets_gemini.json").use { inputStream ->
                BufferedReader(InputStreamReader(inputStream)).use { it.readText() }
            }
            val jsonObject = JSONObject(jsonString)
            jsonObject.optString("gemini_api_key", "").trim()
        } catch (e: Exception) {
            Log.e(TAG, "Failed to read secrets_gemini.json: ${e.message}")
            ""
        }
    }

    /**
     * Parses the Gemini API response safely using Kotlinx Serialization.
     */
    private fun parseStoryResponse(jsonString: String): Story {
        ...
        return Story(
            title = storyJson["title"]?.jsonPrimitive?.content ?: "Untitled",
    ...
    /**
     * Calls Gemini API to generate a 30-second Urdu moral story about [topic]
     * and returns a [Story] object.
     */
    suspend fun generateStory(topic: String): Story = withContext(Dispatchers.IO) {
        val apiKey = getApiKey()
        if (apiKey.isEmpty() || apiKey.startsWith("PASTE_YOUR_") || apiKey.startsWith("YOUR_")) {
            throw IllegalStateException("Valid Gemini API key not found in assets/secrets_gemini.json")
        }
